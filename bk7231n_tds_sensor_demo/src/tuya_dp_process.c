...
//设备DPID
#define DPID_TDS_VALUE    101

tds_detection_S  tds_sensor_status = {      
    .water_quality = GOOD
};                                          //ADC采样后根据转换电压值将水质分为好（0-30）一般（30-100）差（>100）三个等级
...
VOID updata_dp_all(VOID)
{
    ...
    water_val = adc_value_get();
    if (water_val >= 0 && water_val <= 30) {
        tds_sensor_status.water_quality = GOOD;
    } else if (water_val > 30 && water_val <= 100) {
        tds_sensor_status.water_quality = COMMON;
    } else {
        tds_sensor_status.water_quality = POOR;
    }
    
    dp_arr[0].dpid = DPID_TDS_VALUE;
    dp_arr[0].type = PROP_ENUM;
    dp_arr[0].time_stamp = 0;
    dp_arr[0].value.dp_enum = tds_sensor_status.water_quality;
